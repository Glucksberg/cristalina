...
describe("End-to-end lifecycle", () => {
  it("LOG -> PROPOSE -> curate -> ratify -> compile -> bootstrap", async () => {
    ...
    // 2. PROPOSE based on observations
    await executeOperation(store, {
      op: "PROPOSE", type: "revise_preference", target: "core/preferences/communication.yaml",
      reason: "User prefers concise replies.", supporting_events: ["evt-test-001"],
      confidence: 0.68, privacy_scope: "owner_private",
      question_candidate: "Do you prefer concise answers by default?",
    });

    // 3. Seed a core object to confirm against
    ...
    const result = await applyRatification(store, {
      ...
      questionToTarget: new Map([[packet!.questions[0].id, "fact-seed-001"]]),
    });
    ...
    expect(auditLog.length).toBeGreaterThanOrEqual(4); // LOG x2 + PROPOSE + CONFIRM
    expect(auditLog.some((e) => e.operation === "LOG")).toBe(true);
    expect(auditLog.some((e) => e.operation === "PROPOSE")).toBe(true);
    expect(auditLog.some((e) => e.operation === "CONFIRM")).toBe(true);
  });
});
